...
| Konsolidace tohoto repa | 2 414 → 842 (kolo 1) → 774 řádků (kolo 2); 209 položek → 187 (176 pravidel, 11 postupů); vyřazeno 19 (2 v kole 1, 17 v kole 2); ze 4 duplicit `tech.md` × playbook bodu 7: round trip UTF-8 vyřešen (1 položka přepsána na postup s Důkazem: tech.md, sekce „Pasti prostředí"); `Set-StrictMode` nad vrácenými kolekcemi, Git Bash vs. WSL stub a `git rev-parse --git-path` nejsou po kole 1 v playbooku restatovány žádnou položkou (fakt jen v `tech.md`); dva schválené přepisy (`$host`, ruční běh hooku z Git Bash) byly no-op, protože jejich premisa neplatila — obě položky ponechány beze změny; přesunuto do `tech.md` 0; převedeno do kódu 0 (žádná položka neříká přesně lekci sady hygieny; `tests-hygiene.tests.ps1` hlídá obojí); výsledek: nad prahem, ráčna `baseline: 774` a eskalační report `proposals/next/design_root_playbook_eskalace.md` | běhy `c1` (9d54d10) a `c2` (36988ac), 2026-09-24 |
