# Dependency-free assertion helper for epic-graph tests.
...
# Runs epic-graph.ps1 out-of-process; returns @{ Out=<stdout string>; Code=<exit code> }.
function Invoke-Graph([string[]] $ScriptArgs) {
    $graph = Join-Path $PSScriptRoot '..\scripts\epic-graph.ps1'
    ...
    $out = & pwsh -NoProfile -File $graph @ScriptArgs 2>&1 | Out-String
