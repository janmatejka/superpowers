...
> Contract core: [UMS_MEMORY_BANK_CONTRACT](../shared/UMS_MEMORY_BANK_CONTRACT.md). Read it before acting — MB_ROOT resolution, the proposal pair model, and fail-closed rules.
...
- work-item archival: on completion the design half (design_<slug>.md, legacy proposal_<slug>-design.md) moves proposals/active/→proposals/completed/ and the plan half (plan_<slug>.md, legacy proposal_<slug>.md) is deleted; on abandon both halves move proposals/active/→proposals/abandoned/.
...
- When `ACTIVE_WORK`, consider the current plan task (plan checkboxes and `.superpowers/sdd/<plan-basename>/progress.md`) – the commit message should reflect which task is being completed.
