...
# Builds the merged brief.md content: brief content, two blank lines, the
# "Produktový pohled" heading, one blank line, then the demoted product body.
# Writes UTF-8 without BOM — the same rule Update-Links follows above.
function Merge-ProductIntoBrief([string] $BriefFile, [string] $ProductFile) {
    $briefLines = @(Get-Content -LiteralPath $BriefFile)
    $productLines = @(Get-Content -LiteralPath $ProductFile)
    $productBody = Get-ProductBody $productLines
    ...
    $merged.Add('')
    foreach ($ln in $productBody) { $merged.Add($ln) }
...
foreach ($root in $mbRoots) {
    $absRoot = Join-Path $RepoPath ($root -replace '/', '\')
    ...
    if ($hasTasks -and $hasPlaybook) {
        ...
        # The whole MB is skipped — nothing about it is touched this run.
        ...
        if ($needsProduct -and $needsTasks) {
            $action = 'sloučit product.md + přejmenovat tasks.md'
        } elseif ($needsProduct) {
            $action = 'sloučit product.md'
        } elseif ($needsTasks) {
            $action = 'přejmenovat tasks.md'
        } else {
            $action = 'beze změny (hotovo)'
        }

        if ($Apply) {
            if ($needsProduct) {
                if ($hasBrief) {
                    Merge-ProductIntoBrief $briefPath $productPath
                    ...
                    $merged = $true
                    $notes.Add('product.md sloučen do brief.md.')
                } else {
                    # No brief.md to merge into: a plain rename. The file's own
                    # H1 still says "Product" — a human needs to look at it.
                    Invoke-RepoGitMutation @('mv', (Get-RelPath $productPath), (Get-RelPath $briefPath)) "mv $root/product.md -> brief.md"
                    Add-Finding 'PRODUCT BEZ BRIEFU' 'VAROVÁNÍ' "${root}: product.md přejmenován na brief.md, chybí originální brief.md — nadpis souboru zůstal „Product", zkontrolujte ručně."
                    $notes.Add('product.md přejmenován na brief.md (chybí originální brief.md, nadpis zůstal beze změny).')
                }
            }
            if ($needsTasks) {
                Invoke-RepoGitMutation @('mv', (Get-RelPath $tasksPath), (Get-RelPath $playbookPath)) "mv $root/tasks.md -> playbook.md"
                $renamed = $true
                $notes.Add('tasks.md přejmenován na playbook.md.')
            }
        }
    }
